...
/**
*  file    SC_PWM_signal.h
*
*  brief   signal base class for signal sc_pwm
*
...
#ifndef SC_PWM_SIGNAL_H_INCLUDED
#define SC_PWM_SIGNAL_H_INCLUDED
 ...
 *  I-Port
 */
typedef RC_t (*SC_PWM_driverIn_t)(SC_PWM_data_t * const);

/**
 *  Definition of function types for driver interfaces.
 *  O-Port
 */
typedef RC_t (*SC_PWM_driverOut_t)( const SC_PWM_data_t * const);
...
/**
 * Instances of the struct SC_PWM_cfg_t represent the signal object configuration.
 */
struct SC_PWM_cfg_s
{
    /**
     ...
     */
    SC_PWM_driverIn_t const inDriver;

    /**
     * Function pointer to the output driver table
     */
   SC_PWM_driverOut_t const outDriver;
...
typedef const struct SC_PWM_cfg_s SC_PWM_cfg_t;
...
/**
 * Instances of the struct SC_PWM_t represent the signal objects.
...
{                                                                                                                      
     ...
     */
	const SC_PWM_cfg_t * const    pSigCfg;

};                                                                                                                     
typedef struct SC_PWM_s SC_PWM_t;
 ...
 *
 * @param  SC_PWM_t * const signal: Pointer to the signal object which will be initialised
 * @param const sc_pwm_data_t * const value: Pointer to initial value of the application data
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
inline RC_t RTE_SC_PWM_init( SC_PWM_t * const signal, const SC_PWM_data_t * const value )
{                                                                                                                      
 ...
 *
 * @param  SC_PWM_t * const signal: Pointer to the signal object
 * @return  sig_status_t: The current signal status
 *
 **/
inline RTE_signalStatus_t RTE_SC_PWM_getStatus( const SC_PWM_t * const signal )
{
 ...
 *
 */
inline RC_t RTE_SC_PWM_setStatus( SC_PWM_t * const signal, const RTE_signalStatus_t status )
{
 ...
 */
inline uint32_t RTE_SC_PWM_getAge( const SC_PWM_t * const signal )
{
   return signal->age;
}
 ...
 * @return void
 */
inline void RTE_SC_PWM_incAge(  SC_PWM_t * const signal , uint32_t increment)
{
	   //Hack....
 ...
 *
 * @param  SC_PWM_t * const signal: Pointer to the signal object
 * @return sc_pwm_data_t: current value of the application data buffer
 *
 **/
inline SC_PWM_data_t RTE_SC_PWM_get( const SC_PWM_t * const signal )
{               
 ...
 *
 * @param  SC_PWM_t * const signal: Pointer to the signal object
 * @param  SC_PWM_data_t* data: Pointer to the return data Pointer to the signal object
 * @return RC_SUCCESS
 *
 **/
inline RC_t RTE_SC_PWM_getThreadSafe( const SC_PWM_t * const signal, SC_PWM_data_t * const data )
{    
 ...
 *
 * @param  SC_PWM_t * const signal: Pointer to the signal object
 * @param  sc_pwm_data_t const value: value for the application data buffer
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
inline RC_t RTE_SC_PWM_set(SC_PWM_t * const signal, const SC_PWM_data_t value  )
{
 ...
 *
 */
inline RC_t RTE_SC_PWM_pullPort( SC_PWM_t * const signal )
{                                                                                                                      
   RC_t ret = RC_SUCCESS;
   ...
   {
	  SC_PWM_data_t value = signal->value;
      ret = signal->pSigCfg->inDriver(&value);
                                                                                                                       
      if ( ret == RC_SUCCESS )                                                                                         
      {
    	  //Critical Section
    	  RTE_SC_PWM_set(signal, value);
      }
      else
      {
		  //Signal is invalid. Error event (if present) will be fired
		  RTE_SC_PWM_setStatus(signal, RTE_SIGNALSTATUS_INVALID);
      }
 ...
 *
 **/
inline RC_t RTE_SC_PWM_pushPort( const SC_PWM_t * const signal )
{                                                                                                                      
   RC_t ret = RC_ERROR;
